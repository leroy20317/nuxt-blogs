<template>
  <div class="container">
    ...
    <section class="list">
      ...
      <template v-else>
        <div v-for="(val, key, idx) in data.data" :key="idx" class="year-list">
          <ul v-for="(vals, keys, idxs) in val" :key="idxs" class="mon-list">
          ...
          .then(res => {
            if (res.status === 1) {
              ...
              Object.keys(result).forEach(item => {
                const [childA, childB] = [this.data.data[item], result[item]];
                if (childA) {
                  Object.keys(childB).map(
                    i =>
                      (this.data.data[item][i] = childA[i]
                        ? childA[i].concat(childB[i])
                        : childB[i])
                  );
                } else {
