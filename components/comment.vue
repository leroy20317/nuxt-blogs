<template>
  <div class="comment">
    <section class="comment-section" style="transition: all 0.3s">
      <div class="comment-form">
        ...
        <!-- submit loading -->
        <div class="bottom">
          <button :class="status === 9 ? 'active' : ''" type="button" @click="submitVerify">
            SUBMIT
          </button>
      ...
      <template v-if="comment.total > 0">
        <h2>
          <span>Comment List</span><span>({{ comment.total }})</span>
        </h2>
    ...
    <div>
      <puzzle-verification
        :on-success="verifyResult"
        :verification-show="isVerification"
        block-type="puzzle"
        @clone="verifyResult"
      />
    </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    submit() {
      ...
      const commentImage = localStorage.getItem('comment-image');
      let image = 0;
